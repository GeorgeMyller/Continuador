...
class MonitoringManager:
    ...
    def _check_emergency_stop(self) -> bool:
        """
        Verifica se o mouse está no canto superior esquerdo (parada de emergência)
        ...
        try:
            ...
            emergency_zone = MONITORING_CONFIG.get("emergency_zone_size", 20)

            # Considera canto superior esquerdo como área de emergência
            if x <= emergency_zone and y <= emergency_zone:
                return True
        except Exception:
            # Se não conseguir obter posição, continua normalmente
            pass
        return False
    ...
    def _monitoring_cycle(self) -> None:
        ...
        try:
            # Verificar parada de emergência ANTES de qualquer operação
            ...
            if button_info:
                # Verificar emergência antes do clique também
                if self._check_emergency_stop():
                    self._handle_emergency_stop()
                    return
